...
from analyst.models import AnalystNote, KeyFact, ManagementClaim, WhatChanged
...
class RecordedIHCAnalystEngine:
    model_name = "recorded-pass-1-fixture"
    ...
    def analyse(self, announcement, prior_context):
        ...
        return AnalystNote(
            ...
            impact_score=2,
            impact_level="medium",
            headline="Large nil-cost incentive award; shareholder impact depends on vesting",
            takeaway=(
                "The company granted 2,759,141 nil-cost options, including 1,822,268 to "
                "the CEO and CFO. The three-year LTIP element is performance-conditioned, "
                "while FY26 bonus awards vest immediately."
            ),
            key_facts=[
                KeyFact(
            ...
            what_changed=WhatChanged(
                before="Coverage initiated; no prior Smallcaps.ai remuneration record is available.",
                today=(
                    "The company issued annual LTIP and FY26 bonus awards as nil-cost options, "
                    "with the LTIP subject to EBITDA and cash-conversion conditions."
                ),
                read_through=(
                    "The announcement creates prospective dilution and aligns part of the award "
                    "with operating and cash-conversion delivery, but the precise dilution percentage "
                    "is not disclosed in the source excerpt."
                ),
                coverage_status="building",
            ),
            analyst_view=(
                "The absolute award size is meaningful and should be assessed against issued share "
                "capital before drawing a dilution conclusion. Performance conditions improve "
                "alignment for the LTIP element; immediate vesting of FY26 bonus awards is a separate "
                "remuneration outcome rather than evidence of improved trading."
            ),
            supports_case=[
            ...
            management_claims=[
                ManagementClaim(
                    claim=(
                        "LTIP awards will vest on the third anniversary subject to minimum adjusted "
                        "EBITDA and EBITDA-to-operating-cashflow conversion targets."
                    ),
                    target_date="31 January 2029",
                    status="open",
                    evidence="Grant of awards under Long-Term Incentive Plan, 7 August 2026.",
                )
            ],
            watch_items=[
...
def test_real_rns_fixture_runs_end_to_end_and_versions_analysis() -> None:
    ...
    text = (Path(__file__).parent / "fixtures" / "ihc_ltip_2026.txt").read_text(
        encoding="utf-8"
    )
    ...
    assert current["analysis_version"] == "aim-intelligence-foundation-1.0"
