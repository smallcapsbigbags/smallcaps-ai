...
from analyst.evidence import normalise_source_url
...
def build_manual_announcement(*, ticker: str, company: str, published_at: datetime, title: str, text: str, source_url: str = "", rns_type: str = "Other", categories: list[str] | None = None, source_id: str = "", isin: str = "") -> AnnouncementInput:
    clean_ticker = normalise_ticker(ticker)
    clean_title = (title or "").strip()
    clean_text = (text or "").strip()
    clean_url = normalise_source_url(source_url)
    if source_url.strip() and not clean_url:
        raise ValueError("Original RNS URL must be an absolute http:// or https:// URL")
    if not clean_title:
    ...
    if not source_id:
        identity = "|".join([clean_ticker, published_at.isoformat(), clean_title, clean_url])
