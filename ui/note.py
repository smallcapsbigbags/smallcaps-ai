    ...
    with st.container(key="analyst-note"):
        ...
        for fact in note.get("key_facts") or []:
            ...
            if basis == "calculated":
                ...
                if calculation_note:
                    calculations.append(f"{fact.get('label') or fact.get('metric')}: {calculation_note}")
            elif basis == "reported":
            ...
            facts.append([
                ...
                str(fact.get("previous_value") or fact.get("comparator") or ""),
                source_label,
            ])
        st.markdown('<div class="sca-section"></div><div class="sca-section-title">Key numbers</div>', unsafe_allow_html=True)
        if facts:
            st.markdown(_html_table(["Metric", "Current", "Previous / comparator", "Source"], facts, numeric_columns={1, 2}), unsafe_allow_html=True)
        else:
            st.caption("No decision-useful figures were disclosed.")
        if calculations:
            st.markdown('<div class="sca-section-title">How Smallcaps.ai calculated it</div>' + _list_markup(calculations, ""), unsafe_allow_html=True)
        ...
        st.markdown(f'<div class="sca-section"><div class="sca-section-title">Smallcaps.ai view</div><div class="sca-analyst-view">{html.escape(str(note.get("analyst_view") or ""))}</div><div class="sca-body">This is Smallcaps.ai analysis, not a company-reported fact.</div></div>', unsafe_allow_html=True)

        cols = st.columns(2)
        with cols[0]:
            st.markdown('<div class="sca-section"><div class="sca-section-title">What supports the case</div>' + _list_markup(list(note.get("supports_case") or []), "No new supporting evidence identified.") + '</div>', unsafe_allow_html=True)
        with cols[1]:
            st.markdown('<div class="sca-section"><div class="sca-section-title">What could go wrong</div>' + _list_markup(list(note.get("challenges_case") or []), "No new challenge identified.") + '</div>', unsafe_allow_html=True)
        ...
        if guidance:
            st.markdown(_html_table(["Metric", "Period", "Current position", "Status", "Previous"], guidance, numeric_columns={2, 4}), unsafe_allow_html=True)
        else:
        ...
        if missing:
            st.markdown('<div class="sca-section"><div class="sca-section-title">What is missing</div>' + _list_markup(missing, "") + '</div>', unsafe_allow_html=True)
        if mismatch:
            st.markdown(f'<div class="sca-section"><div class="sca-section-title">Management wording check</div><div class="sca-body">{html.escape(mismatch)}</div></div>', unsafe_allow_html=True)

        concepts = list(disclosure.get("concept_explanations") or [])
        if concepts:
            st.markdown('<div class="sca-section"></div><div class="sca-section-title">Worth explaining</div>', unsafe_allow_html=True)
