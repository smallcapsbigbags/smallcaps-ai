...
from ui.common import ensure_watchlist, impact_badge, london_now, navigate, price_markup, render_brand, toggle_watchlist
...
def _render_actions(item: dict[str, Any], *, watchlist: set[str], low: bool) -> None:
    ...
    with cols[2]:
        ...
        if st.button("★ Watchlist" if starred else "☆ Watchlist", key=f"watch-{item['source_id']}", use_container_width=True):
...
def _render_item(item: dict[str, Any], *, watchlist: set[str]) -> None:
    ticker = html.escape(str(item["ticker"])); company = html.escape(str(item["company"])); time_text = html.escape(format_time(str(item["published_at"]))); rns_type = html.escape(str(item["rns_type"])); headline = html.escape(str(item["headline"])); takeaway = html.escape(str(item["takeaway"])); impact = impact_badge(str(item["impact_colour"]), str(item["impact_level"])); price = price_markup(item.get("price")); score = int(item.get("impact_score") or 1); low = score == 1
...
def render_feed(repository: ProductRepository, settings: Settings) -> None:
    ...
    with control_cols[0]:
        search = st.text_input("Search", placeholder="Ticker, company or announcement", label_visibility="collapsed")
    with control_cols[1]:
        scope = st.selectbox("Scope", ["All AIM", "Watchlist"], label_visibility="collapsed")
    with control_cols[2]:
        sort_label = st.selectbox("Sort", ["Most Impactful", "Latest"], label_visibility="collapsed")
    ticker_filter = watchlist if scope == "Watchlist" else None
    items = repository.list_feed(selected_day, search=search, tickers=ticker_filter, sort="latest" if sort_label == "Latest" else "impact")
    if scope == "Watchlist" and not watchlist:
        st.markdown('<div class="sca-empty">Your watchlist is empty. Add companies from the feed.</div>', unsafe_allow_html=True)
        return
    if not items:
        st.markdown('<div class="sca-empty">No publishable AIM analysis matches this view. The daily ingestion worker may not have completed yet.</div>', unsafe_allow_html=True)
        return
    attention = attention_count(items)
    st.markdown(f'<div class="sca-summary"><strong>{attention}</strong> announcement{"s" if attention != 1 else ""} warrant attention · {len(items)} publishable records</div>', unsafe_allow_html=True)
    for item in items:
