...
def render_company(
    ...
    *,
    ...
    clean_ticker = html.escape(str(history["ticker"]))
    company = html.escape(str(history["company"]))
    count = int(history.get("announcement_count") or 0)
    coverage_since = str(history.get("coverage_since") or "")
    coverage_copy = (
        f"{count} analysed announcement{'s' if count != 1 else ''}"
        + (
            f" · coverage since {format_day(coverage_since)}"
            if coverage_since
            else ""
        )
    )
    st.markdown(
        ...
        <div class="sca-company-banner">
          <div class="sca-eyebrow">Company RNS History</div>
          <h1 class="sca-note-title"><span class="sca-ticker">{clean_ticker}</span> · {company}</h1>
          <div class="sca-body">{html.escape(coverage_copy)}</div>
        </div>
        <div class="sca-building">
          Company Intelligence is building from point-in-time daily coverage.
          Smallcaps.ai is not backfilling a synthetic 12-month thesis for V1.
        </div>
        """,
    ...
    for item in announcements:
        ...
        price = html.escape(format_price_change(item.get("price")))
        st.markdown(
            f"""
            <div class="sca-feed-item">
              <div class="sca-meta">
                <span>{html.escape(format_day(published))}</span>
                <span>{html.escape(format_time(published))}</span>
                <span>·</span>
                <span>{rns_type}</span>
                <span class="sca-meta-spacer"></span>
                {impact}
                <span class="sca-price">{price}</span>
              </div>
              <div class="sca-headline">{headline}</div>
              <div class="sca-takeaway">{html.escape(str(item.get("takeaway") or ""))}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        action_cols = st.columns([1.2, 1.1, 6])
        with action_cols[0]:
            if st.button(
                "Analyst Note →",
